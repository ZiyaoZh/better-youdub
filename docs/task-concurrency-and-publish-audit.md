...
Web 后台队列：非 GPU 步骤走 5 worker 并发，非配音 GPU 步骤走 3 worker 并发，配音生成步骤走单 worker 队列。
...
- Web executor 改为按步骤分流：非 GPU worker 使用 `max_workers=5` 并发执行下载、
  翻译、字幕、合成、发布包和上传等步骤。
- GPU worker 使用 `max_workers=3` 并发执行 `separate-audio`、`transcribe*`、
  `transcribe-tts` 等非配音独占步骤。
- `tts` 和 `redub-tts` 使用单 worker 配音队列串行执行，避免共享配音模型并发。
- `run-all` 保持同一任务内步骤顺序，遇到 GPU 或配音生成步骤时以单步骤为单位进入
  对应队列等待完成。同一任务仍由 `_RUNNING` 和 `.task.lock` 互斥。
- `_RUNNING` 仅保留未完成 Future；后台 job 完成后通过 done callback 移除，避免失败
  Future 的 traceback 长期持有 GPU 模型或 tensor 引用。
