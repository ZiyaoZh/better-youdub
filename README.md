...
Web UI 的参数配置是任务级的。URL 表单支持两种入口：“下载并创建”会按表单下载
参数直接下载，下载成功后创建或复用任务；“先创建任务”会先生成一个 URL 占位任务，
此时可以在任务详情里保存下载、WhisperX、翻译、TTS、合成、发布包和 Bilibili
参数，然后在下载步骤卡片点击“下载”。下载完成后，后端会保留原任务 ID 和已保存
参数，并把标题、作者、来源 key、稳定任务目录和下载产物回填到该任务。表单里可填写
cookies 文件路径，也可一次性粘贴 Netscape 格式 cookies 内容，后端会写入
`YOUDUB_COOKIES_PATH` 指向的本地文件并用于下载。开启“下载完成自动运行全流程”后，
URL 下载成功会立即在同一任务上继续运行完整链路，等同于自动点击该任务的“运行完整链路”。
