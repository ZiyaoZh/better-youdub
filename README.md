...
Web UI 的参数配置是任务级的。URL 创建表单提供下载参数，因为下载会在任务创建前
执行；可在表单里填写 cookies 文件路径，也可一次性粘贴 Netscape 格式 cookies
内容，后端会写入 `YOUDUB_COOKIES_PATH` 指向的本地文件并用于本次下载。任务创建后，
步骤卡片上的设置入口会打开“任务参数”抽屉，保存该任务自己的下载、WhisperX、翻译、
TTS、合成、发布包和 Bilibili 参数快照。空的密钥字段会继续回退到环境变量或运行时
配置文件；为单个任务填写密钥时，接口响应只返回 `********`，后续保存该掩码会保留原值。
