...
- FastAPI Web UI：创建 URL/本地/上传任务、查看任务状态、运行单步或完整链路、下载产物、配置 cookies/OpenAI/yt-dlp
...
中固定按 dry-run 执行；真实上传仍使用 CLI 的 `--publish-confirm` 显式确认入口。
...
文件路径，写出 `bilibili.dry-run.json`；真实上传需要设置 Bilibili 凭证，并显式
传入 `--publish-confirm`：
