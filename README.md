...
Web UI 的“存储”页面按需统计任务视频、中间产物和遗留上传占用。可以清理单个任务，也可以批量清理所有非活动任务；批量操作会跳过正在运行、排队、定时发布或被其他进程锁定的任务。清理会保留 `tasks.json` 中的任务记录以及任务目录内的 `task.json` 和 `.task.lock`，任务状态、步骤、参数和错误信息不会改变；视频与中间产物删除后不可恢复。浏览器新上传的视频在复制进任务目录后会立即删除临时副本。
